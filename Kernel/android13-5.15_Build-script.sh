mkdir kernel
cd kernel
repo init -u https://android.googlesource.com/kernel/manifest.git -b common-android13-5.15-lts --depth=1
repo sync --force-sync
ln -s common/build.config.gki.aarch64 build.config
cd common
sed -i "s|Image.lz4|Image.gz|g" build.config.gki.aarch64
wget https://gist.githubusercontent.com/sarthakroy2002/4e1c6bc5b5745168f1d47f4eb1bb25d6/raw/1a70b2aa9c401262de2dad03f8231b3f03d5325d/uncommitted.patch
patch -p1 < uncommitted.patch
cd ..
echo "Compile starting..."
bash build/build.sh
echo "Compilied..."
git clone https://github.com/Koushikdey2003/AnyKernel3 -b android13-5.15 
cp -r out/android13-5.15/dist/Image.gz AnyKernel3
cd AnyKernel3
zip -r9 android13-5.15-AnyKernel3.zip ./*
